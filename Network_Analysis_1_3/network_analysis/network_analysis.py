...
class NetworkAnalysis:
                        ...
                        if str(oriente) == "Yes" :
                            ...
                            if ok2 :
                                if met == "Total interaction constrained" :
                                    res, val = gravi.gravitaire_oriente_uncomplete(G,table,dist)
                                if met == "Double Constrained" :
                                    num,ok3 = QInputDialog.getInt(None,"Iteration","Enter the number of iterations : ", 100)
                                    if ok3 :
                                        res, val = gravi.gravitaire_oriente_uncomplete2(G,table,dist,num)
                                        b = list(res)
                                        QMessageBox.information(None, " Message : ", "R = " + str(val[0][1]))
                        if str(oriente) == "No" :
                            met,ok2 = QInputDialog.getItem(None,"Method","Choose your method : ", ["Total interaction constrained","Double Constrained"], editable = False)
                            if ok2 :
                                if met == "Total interaction constrained" :
                                    res, val = gravi.gravitaire_uncomplete(G,table,dist)
                                if met == "Double Constrained" :
                                    num,ok3 = QInputDialog.getInt(None,"Iteration","Enter the number of iterations : ", 100)
                                    if ok3 :
                                        res, val = gravi.gravitaire_uncomplete2(G,table,dist,num)
                                        b = list(res)
                                        QMessageBox.information(None, " Message : ", "R = " + str(val[0][1]))
                if testzero == "Yes" :
                    geom = []
                    ...
                    if str(oriente)=="No" :
                        ...
                        if ok2 :
                            if met == "Total interaction constrained" :
                                res, val = gravi.gravitaire(Gnew,tablenew,table,dist,dist2)
                            if met == "Double Constrained" :
                                num,ok3 = QInputDialog.getInt(None,"Iteration","Enter the number of iterations : ", 100)
                                if ok3 :
                                    res, val = gravi.gravitaire2(Gnew,tablenew,table,dist,dist2,num)
                                    b2 = list(res)
                                    QMessageBox.information(None, " Message : ", "R = " + str(val[0][1]))
                    if str(oriente)=="Yes":
                        tablenew = []
                        ...
                        if ok2 :
                            if met == "Total interaction constrained" :
                                res, val = gravi.gravitaire_oriente(Gnew,tablenew,table,dist,dist2)
                            if met == "Double Constrained" :
                                num,ok3 = QInputDialog.getInt(None,"Iteration","Enter the number of iterations : ", 100)
                                if ok3 :
                                    res, val = gravi.gravitaire_oriente2(Gnew,tablenew,table,dist,dist2,num)
                                    b2 = list(res)
                                    QMessageBox.information(None, " Message : ", "R = " + str(val[0][1]))
  ...
  def calculc(self):
        ...
        self.dlg2.ui.comboBox_4.addItems(["No","Yes"])
        self.dlg2.ui.comboBox_5.addItems(fields2)
        self.dlg2.ui.comboBox_6.addItems(["Louvain method","Edge Betweenness Cluster","Structural Equivalence", "Blockmodeling", "Spatial Cluster"])
  ...
  def runf(self): 
        ...
        self.dlg.ui.comboBox.addItems(['Tables to Graph','Planar Graph', 'Correct ID', 'Largest connected component', 'Generate Subgraphs', 'Delete Isolated Nodes/Edges', 'To Undirected Graph','Drawing Algo', 'Drawing Curved Arrow'])
