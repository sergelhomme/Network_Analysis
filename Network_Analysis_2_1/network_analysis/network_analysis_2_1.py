...
class NetworkAnalysis:
  ...
  def calcul2d(self):
        ...
        if indicateur == "All Shortest Paths (Distancier)":
            ...
            chn1, ok1 = QInputDialog.getItem(None,"Start Field","Choose Start Node Field : ", f, editable = False)
            if ok1 :
                chn2, ok2 = QInputDialog.getItem(None,"End Field","Choose End Node Field : ", f, editable = False)
                if ok2 :
                    sstart = []
                    ssend = []
                    sstart2 = []
  ...
  def calcul3flc(self):
        ...
        if str(poids)=="Yes":
         ...
         G.add_weighted_edges_from(table)
        t=nx.number_of_nodes(G)
        res = max(nx.connected_component_subgraphs(G), key=len)
        trires = sorted(res)
        v = QgsVectorLayer("LineString?crs="+str(epsg), "Edges", "memory")
        p = v.dataProvider()
        p.addAttributes( [QgsField("ID", QVariant.Int)] )
