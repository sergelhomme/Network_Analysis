...
class NetworkAnalysis:
  ...
  def run(self):
        ...
        self.dlg.ui.buttonBox.accepted.connect(self.calcul)
        self.dlg.show()
  ...
  def calcul(self):
        ...
        if ok :
            if str(typ) == "Nodes" :
                ...
                self.dlg2.ui.comboBox_6.addItems(["Degree", "Closeness", "Fareness", "Harmonic", "Betweenness", "Clustering", "Eigenvector", "Laplacian", "Square_clustering", "Eccentricity", "Page_rank", "Average_neighbor_degree", "Hub_dependence", "Core_Number", "Closeness_vitality"])
                self.dlg2.ui.comboBox_7.addItems(["Yes","No"])
                self.dlg2.ui.buttonBox.accepted.connect(self.calcul2)
                self.dlg2.show()
            if str(typ) == "Edges" :
                self.dlg2 = testDialog2(self.iface.mainWindow())
                self.dlg2.ui.comboBox.addItems(fields)
                ...
                self.dlg2.ui.comboBox_7.addItems(["Yes","No"])
                self.dlg2.ui.buttonBox.accepted.connect(self.calcul2arc)
                self.dlg2.show()
  ...
  def calcul2(self):
        ...
        if str(indicateur)=="Eigenvector" and str(oriente)=="No" and str(norm)=="Yes" :
            b=nx.eigenvector_centrality(G, max_iter=1000000000000000000000000000000000)
        if str(indicateur)=="Eigenvector" and str(oriente)=="Yes" and str(norm)=="Yes" :
            b=nx.eigenvector_centrality(G, max_iter=1000000000000000000000000000000000)
            G2=nx.reverse(G, copy=True)
            b2=nx.eigenvector_centrality(G2, max_iter=1000000000000000000000000000000000)
        ...
        if str(indicateur)=="Closeness" and str(poids)=="Yes" and str(norm)=="Yes":
            b=nx.closeness_centrality(G,distance='weight')
        if str(indicateur)=="Harmonic" and str(poids)=="No" and str(norm)=="No":
            b=nx.harmonic_centrality(G, distance=None)
        if str(indicateur)=="Harmonic" and str(poids)=="Yes" and str(norm)=="No":
            b=nx.harmonic_centrality(G,distance='weight')
        if str(indicateur)=="Harmonic" and str(poids)=="No" and str(norm)=="Yes":
            b=nx.harmonic_centrality(G, distance=None)
        if str(indicateur)=="Harmonic" and str(poids)=="Yes" and str(norm)=="Yes":
            b=nx.harmonic_centrality(G,distance='weight')
        ...
        if str(indicateur)=="Hub_dependence" and str(oriente)=="No" and str(norm)=="Yes" and str(poids)=="Yes":
            ...
            for j in range(t):
               num=j+1
               listarc=list(G.edges(int(num),data=True))
               b2=[0]*(t+1)
               tot=0
        ...
        if str(indicateur)=="Hub_dependence" and str(oriente)=="Yes" and str(norm)=="Yes" and str(poids)=="Yes":
            ...
            for j in range(t):
               num=j+1
               listarcout=list(G.out_edges(int(num),data=True))
               b2=[0]*(t+1)
               tot=0
            ...
            for j in range(t):
               num=j+1
               listarcin=list(G.in_edges(int(num),data=True))
               b2=[0]*(t+1)
               tot=0
            ...
            try:
              for i in range(len(G.nodes())):
                 for j in range(len(G.nodes())):
                     b2[i]=b2[i]+(float(spl[i+1][j+1])/(2*(float(t)-1)))
                     b2[j]=b2[j]+(float(spl[i+1][j+1])/(2*(float(t)-1)))
              err=0
            except :
  ...
  def calcul2arc(self):
        ...
        if str(indicateur)=="Indice de cohesion" and str(norm)=="Yes" and str(poids)=="No":
            ...
            for i in range(len(table)):
                noeud1=table[i][0]
                noeud2=table[i][1]
                neib1=list(G.neighbors(noeud1))
                neib2=list(G.neighbors(noeud2))
                val=0
            ...
            if ok1 :
                ...
                if testzero == "Yes" :
                    ...
                    while fit2.nextFeature(feat2):
                        geom = geom + [feat2.geometry().asWkt()]
        ...
        if b == 77777 and str(indicateur) == "Gravity" and b2 != 0 :
           epsg = aLayer.sourceCrs().authid()
           vl = QgsVectorLayer("LineString?crs="+str(epsg), "Edges", "memory")
           pr = vl.dataProvider()
           vl.startEditing()
  ...
  def runb(self):
        ...
        self.dlg.ui.comboBox.addItems(lay)
        self.dlg.ui.comboBox_2.addItems(lay)
        self.dlg.ui.buttonBox.accepted.connect(self.calculb)
        self.dlg.show()
  ...
  def calculb(self):
        ...
        self.dlg2.ui.buttonBox.accepted.connect(self.calcul2b)
        self.dlg2.show()
  ...
  def calcul2b(self):
        ...
        if str(indicateur)=="Fareness_increase":
            ...
            if err==0:
                 for l in range(len(nd)):
                     ...
                     edg = list(nx.edges(G,l+1))
                     for l2 in range(len(edg)):
                         dep=edg[l2][0]
                         arr=edg[l2][1]
                         G2[dep][arr]['weight']=10000000000000
                     spl2 = dict(nx.shortest_path_length(G2, weight='weight'))
                     totper = 0
                     for l2 in range(len(nd)):
                         for l3 in range(len(nd)):
                             if l2!=l:
                                 if l3!=l:
        ...
        if str(indicateur)=="Error_attack":
             if str(oriente)=="No":
                 ...
                 self.dlg3.ui.buttonBox.accepted.connect(self.calcul3)
                 self.dlg3.show()
             if str(oriente)=="Yes":
                 QMessageBox.information(None, " Message : ", "Currently Graph needs to be undirected.")
                 b=77777
  ...
  def calcul3(self):
        ...
        if str(indicateur)=="Error_attack" and testmatplotview == 1 :
                 ...
                 b=[0.00000]*(int(len(G.nodes())*tx+1))
                 b3=[0.00000]*(int(len(G.nodes())*tx+1))
                 b4=[0.00000]*(int(len(G.nodes())*tx+1))
                 x=[0.00000]*(int(len(G.nodes())*tx+1))
                 cen=nx.betweenness_centrality(G,weight=True)
                 cens=sorted(cen.items(), reverse=True, key=operator.itemgetter(1))
                 cen3=dict(nx.degree(G))
                 cens3=sorted(cen3.items(), reverse=True, key=operator.itemgetter(1))
                 cen4=dict(nx.betweenness_centrality(G,weight=True))
                 cens4=sorted(cen4.items(), reverse=True, key=operator.itemgetter(1))
                 for l in range(int((len(G.nodes()))*tx)) :
                          nbconnex=0
                          it=cens[l][0]
                          edg = list(nx.edges(G,it))
                          for l2 in range(len(edg)):
                                    dep=edg[l2][0]
                                    arr=edg[l2][1]
                                    G2[dep][arr]['weight']=10000000000
                          spl2=dict(nx.shortest_path_length(G2, weight='weight'))
                          nbconnex3=0
                          it3=cens3[l][0]
                          edg3=list(nx.edges(G,it3))
                          for l2 in range(len(edg3)):
                                    dep=edg3[l2][0]
                                    arr=edg3[l2][1]
                                    G3[dep][arr]['weight']=10000000000
                          spl3=dict(nx.shortest_path_length(G3, weight='weight'))
                          nbconnex4=0
                          it4=cens4[0][0]
                          edg4=list(nx.edges(G,it4))
                          for l2 in range(len(edg4)):
                                    dep=edg4[l2][0]
                                    arr=edg4[l2][1]
                                    G4[dep][arr]['weight']=10000000000
                          spl4=dict(nx.shortest_path_length(G4, weight='weight'))
                          for l2 in range(len(G.nodes())):
                                   ligne=list(spl2[l2+1].values())
                                   ligne3=list(spl3[l2+1].values())
                                   ligne4=list(spl4[l2+1].values())
                          ...
                          b[l+1]=nbconnex/float((len(G.nodes())*(len(G.nodes())-1)))
                          b3[l+1]=nbconnex3/float((len(G.nodes())*(len(G.nodes())-1)))
                          b4[l+1]=nbconnex4/float((len(G.nodes())*(len(G.nodes())-1)))
                          x[l+1]=(l+1)/float(len(G.nodes()))
                          G4b.remove_node(it4)
                          cen4=nx.betweenness_centrality(G4b,weight=True)
                          cens4=sorted(cen4.items(), reverse=True, key=operator.itemgetter(1))
                          valbar=int(((3*(l+1))*100)/float((len(G.nodes())*(3+nbal2))))
                          self.dlg5.ui.progressBar.setProperty("value", valbar)
                 b5tot=[0.00000]*(int(len(G.nodes())*tx)+1)
                 for m in range(nbal2) :
                      G5=0
                      G5=nx.Graph()
                      b5=[0.00000]*(int(len(G.nodes())*tx)+1)
                      ...
                      for l in range(int((len(G.nodes()))*tx)) :
                          nbconnex5=0
                          it5=cens5[l]
                          edg5=list(nx.edges(G,it5))
                          for l2 in range(len(edg5)):
                                    dep=edg5[l2][0]
                                    arr=edg5[l2][1]
                                    G5[dep][arr]['weight']=10000000000
                          spl5=dict(nx.shortest_path_length(G5, weight='weight'))
                          for l2 in range(len(G.nodes())):
                                   ligne5=list(spl5[l2+1].values())
                                   for l3 in range(len(G.nodes())):
                                            val5=ligne5[l3]
                                            if val5 >=10000000000 :
                 ...
                 p1, = plt.plot(x, b, 'r', linewidth=lignelar2, marker='o', markersize=pttaille2)
                 p2, = plt.plot(x, b3, color='#00bfbf', linewidth=lignelar2, marker='o', markersize=pttaille2)
                 p3, = plt.plot(x, b4, color='#dbc71c', linewidth=lignelar2, marker='o', markersize=pttaille2)
                 p4, = plt.plot(x, b5tot, color = '#51d51d', linewidth=lignelar2, marker='o', markersize=pttaille2)
  ...
  def runc(self):
        ...
        self.dlg.ui.comboBox.addItems(lay)
        self.dlg.ui.comboBox_2.addItems(lay)
        self.dlg.ui.buttonBox.accepted.connect(self.calculc)
        self.dlg.show()
  ...
  def calculc(self):
        ...
        self.dlg2.ui.comboBox_4.addItems(["No","Yes"])
        self.dlg2.ui.comboBox_5.addItems(fields2)
        self.dlg2.ui.comboBox_6.addItems(["Louvain method", "Louvain 2", "Greedy Modularity", "Edge Betweenness Cluster", "Edge Betweenness Cluster 2", "Structural Equivalence", "Blockmodeling", "Connected Components", "Spatial Cluster"])
        self.dlg2.ui.buttonBox.accepted.connect(self.calcul2c)
        self.dlg2.show()
  ...
  def calcul2c(self):
        ...
        if str(indicateur) == "Louvain method" :
          ...
          if str(oriente)=="Yes" :
             QMessageBox.information(None, " Message : ", "Graph needs to be undirected")
        if (indicateur) == "Louvain 2" :
            if str(poids)=="No":
                part = nx.community.louvain_communities(G, weight=None)
                taillen = len(G.nodes())
                aLayer2 = allLayers[int(ind_noeud)]
                provider2 = aLayer2.dataProvider()
                nbcol = int(provider2.fields().count())
                provider2.addAttributes( [QgsField("Louvain", QVariant.Int)] )
                aLayer2.startEditing()
                for i in range(len(part)):
                    l = list(part[i])
                    for j in range(len(l)):
                        aLayer2.changeAttributeValue(int(l[j])-1,nbcol,(int(i)))
                aLayer2.commitChanges()
            if str(poids)=="Yes":
                part = nx.community.louvain_communities(G, weight='weight')
                taillen = len(G.nodes())
                aLayer2 = allLayers[int(ind_noeud)]
                provider2 = aLayer2.dataProvider()
                nbcol = int(provider2.fields().count())
                ...
                for i in range(len(part)):
                    ...
                    for j in range(len(l)):
                        aLayer2.changeAttributeValue(int(l[j])-1,nbcol,(int(i)))
                aLayer2.commitChanges()
        if (indicateur) == "Greedy Modularity" :
            if str(poids)=="No":
                ...
                provider2 = aLayer2.dataProvider()
                nbcol = int(provider2.fields().count())
                provider2.addAttributes( [QgsField("Greedy", QVariant.Int)] )
                aLayer2.startEditing()
                for i in range(len(part)):
                    l = list(part[i])
                    for j in range(len(l)):
                ...
                aLayer2.commitChanges()
            if str(poids)=="Yes":
                part = nx.community.greedy_modularity_communities(G, weight='weight')
                taillen = len(G.nodes())
                aLayer2 = allLayers[int(ind_noeud)]
                provider2 = aLayer2.dataProvider()
                nbcol = int(provider2.fields().count())
        ...
        if (indicateur) == "Edge Betweenness Cluster" :
             ...
             if ok:
                 ...
                 mod = com.modularity(part,G)
                 QMessageBox.information(None, " Message : ", "Modularity : "+str(mod))
        if (indicateur) == "Edge Betweenness Cluster 2" :
             ncmax,ok = QInputDialog.getInt(None,"Cluster","Enter the number of clusters : ", 5)
             if ok:
                 part = nx.community.edge_betweenness_partition(G, ncmax)
                 taillen = len(G.nodes())
                 aLayer2 = allLayers[int(ind_noeud)]
                 provider2 = aLayer2.dataProvider()
                 nbcol = int(provider2.fields().count())
        ...
        if str(indicateur) == "Structural Equivalence" and testscipy == 1 :
            ...
            if ok :
                ...
                adjar = nx.to_numpy_array(G, nodelist=listnode, weight=None)
        ...
        if str(indicateur) == "Blockmodeling" :
            aLayer2 = allLayers[int(ind_noeud)]
            provider2 = aLayer2.dataProvider()
            epsg = aLayer2.sourceCrs().authid()
            epsgb = aLayer.sourceCrs().authid()
            ...
            if ok :
                ...
                if ok2 :
                        ...
                        while fit2.nextFeature(feat):
                            ...
                            try :
                                ...
                                if modenum > 2 :
                                    ...
                                    champ = champ + [[val2]]
                            i = i + 1
                        bk = nx.quotient_graph(G,part,relabel = True)
                        edg = list(bk.edges())
                        v2 = QgsVectorLayer("Point?crs=" + str(epsg), "Nodes", "memory")
                        p2 = v2.dataProvider()
                        ...
                        v = QgsVectorLayer("LineString?crs=" + str(epsgb), "Edges", "memory")
                        p = v.dataProvider()
        ...
        if str(indicateur) == "Connected Components" :
            if str(oriente)=="No":
                ...
                provider2.addAttributes([QgsField(str(indicateur), QVariant.Int)])
                aLayer2.startEditing()
                taille = len(list(G.nodes()))
                for k in range(taille):
                    for j in range(len(res)):
                        try :
                            test = list(res[j]).index(int(k+1))
                            val = j + 1
                        except :
                            "Do Nothing"
  ...
  def rund(self):
        ...
        self.dlg.ui.comboBox.addItems(lay)
        self.dlg.ui.comboBox_2.addItems(lay)
        self.dlg.ui.buttonBox.accepted.connect(self.calculd)
        self.dlg.show()
  ...
  def calculd(self):
        ...
        self.dlg2.ui.buttonBox.accepted.connect(self.calcul2d)
        self.dlg2.show()
  ...
  def calcul2d(self):
        ...
        if indicateur=="Delaunay":
            ...
            a=processing.run('qgis:delaunaytriangulation', parameters)
            ...
            while fit1.nextFeature(feat):
                som1=int(feat.attributes()[0])
                som2=int(feat.attributes()[1])
                som3=int(feat.attributes()[2])
                x1 = geom[som1][0]
                y1 = geom[som1][1]
                x2 = geom[som2][0]
                y2 = geom[som2][1]
        ...
        if indicateur=="Minimum Spanning Tree":
            ...
            if str(oriente)=="No" :
                epsg = aLayer.sourceCrs().authid()
                v = QgsVectorLayer("LineString?crs="+str(epsg), "MST", "memory")
                p = v.dataProvider()
                v.startEditing()
                ...
                for i in range(len(res)):
                    ...
                    for j in range(len(table)):
                        ...
                        val = val + 1
                    fet = QgsFeature()
                    fet.setGeometry(  QgsGeometry.fromWkt( geo[valsave] ) )
        ...
        if indicateur=="Find Cliques":
            ...
            if str(oriente)=="No" :
                ...
                if ok1 :
                    ...
                    fit2 = provider2.getFeatures()
                    while fit2.nextFeature(feat2):
                        tablengeo = tablengeo + [feat2.geometry().asWkt()]
                        tablen = tablen + [feat2.attributes()]
                    cl = list(nx.find_cliques(G))
                    for i in range(len(cl)):
                        if len(cl[i]) >= int(testzero) :
                            epsg = aLayer.sourceCrs().authid()
                            v = QgsVectorLayer("LineString?crs="+str(epsg), "Clique_"+str(i), "memory")
                            p = v.dataProvider()
                            v.startEditing()
                            ...
                            for j in range(len(cl[i])-1):
                                for k in range(len(cl[i])-1-j):
                                    ...
                                    p.addFeatures( [ fet ] )
                                    v.commitChanges()
                            epsg2 = aLayer2.sourceCrs().authid()
                            v2 = QgsVectorLayer("Point?crs="+str(epsg), "Clique_n_"+str(i), "memory")
                            p2 = v2.dataProvider()
                            v2.startEditing()
                            ...
                            for j in range(len(cl[i])):
                                fet = QgsFeature()
                                fet.setGeometry(QgsGeometry.fromWkt(tablengeo[int(cl[i][j])-1]))
        ...
        if indicateur == "All Shortest Paths (Distancier)":
            ...
            chn1, ok1 = QInputDialog.getItem(None,"Start Field","Choose Start Node Field Weight : ", f, editable = False)
            if ok1 :
                chn2, ok2 = QInputDialog.getItem(None,"End Field","Choose End Node Field Weight : ", f, editable = False)
                if ok2 :
                    sstart = []
                    ssend = []
                    sstart2 = []
                    ...
                    while fit2.nextFeature(feat2):
                        ...
                        num = num + 1
                    v = QgsVectorLayer("LineString?crs="+str(epsg), "SPL", "memory")
                    p = v.dataProvider()
                    v.startEditing()
                    ...
                    for i in range(len(sstart2)):
                        for j in range(len(ssend2)):
                            ...
                            if sstart2[i] == ssend2[j] :
                                    val = 0
                            fet = QgsFeature()
                            fet.setGeometry( QgsGeometry.fromPolylineXY([ geompt[sstart2[i]-1] , geompt[ssend2[j]-1] ]) )
        ...
        if indicateur == "Transportation Problem":
            ...
            for i in range(field2.count()):
                f = f + [str(field2[i].name())]
            chn1, ok1 = QInputDialog.getItem(None,"Start Field","Choose Start Node Capacity Field (Demand) : ", f, editable = False)
            if ok1 :
                chn2, ok2 = QInputDialog.getItem(None,"End Field","Choose End Node Capacity Field (Supply) : ", f, editable = False)
                if ok2 :
                    G2 = nx.DiGraph()
                    sstart = []
                    ssend = []
                    ...
                    v = QgsVectorLayer("LineString?crs="+str(epsg), "TSP", "memory")
                    p = v.dataProvider()
                    v.startEditing()
                    v.startEditing()
                    p.addAttributes( [ QgsField("ID_Start", QVariant.Int) ] )
        ...
        if indicateur == "PMedian" :
            ...
            for i in range(field2.count()):
                f = f + [str(field2[i].name())]
            chn1, ok1 = QInputDialog.getItem(None,"Start Field","Choose Start Node Field (Demand) : ", f, editable = False)
            if ok1 :
                chn2, ok2 = QInputDialog.getItem(None,"End Field","Choose End Node Field (Supply) : ", f, editable = False)
                if ok2 :
                    ...
                    if ok3 :
                        ...
                        res = []
                        noeudarr = list(range(len(ssend2)))
                        noeuddep = list(range(len(sstart2)))
                        ...
                        v = QgsVectorLayer("LineString?crs="+str(epsg), "Pmedian", "memory")
                        p = v.dataProvider()
                        v.startEditing()
                        v.startEditing()
                        ...
                        for i in range(len(sstart2)):
                            ...
                            if ssend2[titulaire[jmin]] != sstart2[i]:
                                fet = QgsFeature()
                                fet.setGeometry( QgsGeometry.fromPolylineXY([ geompt[sstart2[i]-1] , geompt[ssend2[titulaire[jmin]]-1] ]) )
  ...
  def rune(self):
        ...
        self.dlg.ui.comboBox.addItems(lay)
        self.dlg.ui.buttonBox.accepted.connect(self.calcule)
        self.dlg.show()
  ...
  def calcule(self):
        ...
        self.dlg2.ui.buttonBox.accepted.connect(self.calcul2e)
        self.dlg2.show()
  ...
  def calcul2e(self):
        ...
        if indicateur == "Correlation (Pearson)"  and testmatplotview == 1:
            ...
            atableglo= np.array(table)
            reg = np.polyfit(atableglo[:,0], atableglo[:,1], 1)
            corr = np.corrcoef(atableglo[:,0], atableglo[:,1])
            self.dlg3 = testMatplot(self.iface.mainWindow())
            ax = self.dlg3.ui.figure.add_subplot(111)
            ax.plot(atableglo[:,0],atableglo[:,1], 'ro')
            minix = min(atableglo[:,0])
            maxix = max(atableglo[:,0])
            ...
            self.dlg3.ui.bouton1.clicked.connect(self.change2)
        ...
        if indicateur == "Distribution" and testmatplotview == 1 :
            ...
            self.dlg3.ui.canvas.draw()
            self.dlg3.ui.bouton1.clicked.connect(self.change)
  ...
  def change(self):
        ...
        if str(valchange) == "Histogram" :
            ...
            if ok:
                ...
                ax = self.dlg3.ui.figure.add_subplot(111)
                ax.hist(atable,num,color='r',density=False)
                ax.set_xlabel(str(champ))
  ...
  def runf(self):
        ...
        self.dlg.ui.buttonBox.accepted.connect(self.calculf)
        self.dlg.show()
  ...
  def calculf(self):
        ...
        if tool == 'Correct ID' :
            ...
            self.dlg2.ui.buttonBox.accepted.connect(self.calcul2fid)
            self.dlg2.show()
        if tool == 'Planar Graph' :
            ...
            if ok :
                if str(pg) == "Create Planar Graph" :
                    ...
                    self.dlg2.ui.comboBox.addItems(lay)
                    self.dlg2.ui.buttonBox.accepted.connect(self.calcul2fpg)
                    self.dlg2.show()
                else :
                    self.dlg2 = testDialog(self.iface.mainWindow())
                    self.dlg2.ui.comboBox.addItems(lay)
                    self.dlg2.ui.comboBox_2.addItems(lay)
                    self.dlg2.ui.buttonBox.accepted.connect(self.calcul2fpgb)
                    self.dlg2.show()
        if tool == 'Tables to Graph' :
            self.dlg2 = testDialog(self.iface.mainWindow())
            laytab = [l.name() for l in QgsProject.instance().mapLayers().values()]
            self.dlg2.ui.comboBox.addItems(laytab)
            self.dlg2.ui.comboBox_2.addItems(laytab)
            self.dlg2.ui.buttonBox.accepted.connect(self.calcul2f)
            self.dlg2.show()
        if tool == 'Edges Table to Graph (Geocoding)' :
            self.dlg2 = testDialogtoolb(self.iface.mainWindow())
            laytab = [l.name() for l in QgsProject.instance().mapLayers().values()]
            self.dlg2.ui.comboBox.addItems(laytab)
            self.dlg2.ui.buttonBox.accepted.connect(self.calcul2fet)
            self.dlg2.show()
        if tool == 'Largest Connected Component' :
            self.dlg2 = testDialog(self.iface.mainWindow())
            self.dlg2.ui.comboBox.addItems(lay)
            self.dlg2.ui.comboBox_2.addItems(lay)
            self.dlg2.ui.buttonBox.accepted.connect(self.calcul2flc)
            self.dlg2.show()
        if tool == 'Generate Subgraphs' :
            self.dlg2 = testDialog(self.iface.mainWindow())
            self.dlg2.ui.comboBox.addItems(lay)
            self.dlg2.ui.comboBox_2.addItems(lay)
            self.dlg2.ui.buttonBox.accepted.connect(self.calcul2fgs)
            self.dlg2.show()
        if tool == 'Generate Cycles' :
            self.dlg2 = testDialog(self.iface.mainWindow())
            self.dlg2.ui.comboBox.addItems(lay)
            self.dlg2.ui.comboBox_2.addItems(lay)
            self.dlg2.ui.buttonBox.accepted.connect(self.calcul2fgc)
            self.dlg2.show()
        if tool == 'Delete Isolated Nodes/Edges' :
            self.dlg2 = testDialog(self.iface.mainWindow())
            self.dlg2.ui.comboBox.addItems(lay)
            self.dlg2.ui.comboBox_2.addItems(lay)
            self.dlg2.ui.buttonBox.accepted.connect(self.calcul2fdin)
            self.dlg2.show()
        if tool == 'To Undirected Graph' :
            self.dlg2 = testDialog(self.iface.mainWindow())
            self.dlg2.ui.comboBox.addItems(lay)
            self.dlg2.ui.comboBox_2.addItems(lay)
            self.dlg2.ui.buttonBox.accepted.connect(self.calcul2ftoun)
            self.dlg2.show()
        if tool == 'Drawing Algo' :
            self.dlg2 = testDialog(self.iface.mainWindow())
            self.dlg2.ui.comboBox.addItems(lay)
            self.dlg2.ui.comboBox_2.addItems(lay)
            self.dlg2.ui.buttonBox.accepted.connect(self.calcul2fdraw)
            self.dlg2.show()
        if tool == 'Drawing Curved Arrow' :
            self.dlg2 = testDialog(self.iface.mainWindow())
            self.dlg2.ui.comboBox.addItems(lay)
            self.dlg2.ui.comboBox_2.addItems(lay)
            self.dlg2.ui.buttonBox.accepted.connect(self.calcul2fcar)
            self.dlg2.show()
  ...
  def calcul2f(self):
        ...
        laytab = [l.name() for l in QgsProject.instance().mapLayers().values()]
        allLayers2 = list(QgsProject.instance().mapLayers().values())
        global ind_noeud
        ind_noeud = laytab.index(str(table_noeud))
        global ind_arc
        ind_arc = laytab.index(str(table_arc))
        global aLayer
        aLayer = allLayers2[int(ind_arc)]
        global provider
        provider = aLayer.dataProvider()
        global field
        field=provider.fields()
        ...
        for i in range(field.count()):
          fields=fields+[str(field[i].name())]
        global nLayer
        nLayer = allLayers2[int(ind_noeud)]
        global provider2
        ...
        self.dlg3.ui.buttonBox.accepted.connect(self.calcul3f)
        self.dlg3.show()
  ...
  def calcul2flc(self):
        ...
        self.dlg3.ui.buttonBox.accepted.connect(self.calcul3flc)
        self.dlg3.show()
  ...
  def calcul2fdraw(self):
     if testmatplotview == 1 :
        ...
        self.dlg3.ui.comboBox_4.addItems(["No","Yes"])
        self.dlg3.ui.comboBox_5.addItems([""]+fields)
        self.dlg3.ui.comboBox_6.addItems(["Fruchterman-Reingold force-directed algorithm", "Kamada Kawai", "Spectral", "Circular", "Spiral", "Force Atlas"])
        self.dlg3.ui.buttonBox.accepted.connect(self.calcul3fdraw)
        self.dlg3.show()
  ...
  def calcul2fpg(self):
        ...
        while fit.nextFeature(feat):
            ...
            if str(geomwkt[0:10])== 'LineString' or str(geomwkt[0:10])== 'LINESTRING' or str(geomwkt[0:10])== 'Linestring' or str(geomwkt[0:10])== 'linestring':
                geomwkt2 = geomwkt.split('(')
                geomwkt3 = geomwkt2[1].split(')')
                geomwkt4 = geomwkt3[0].split(',')
                for i in range(len(geomwkt4)):
                    geomwkt5 = geomwkt4[i].split(' ')
                    if i == 0 :
                        x = float(geomwkt5[0])
                        y = float(geomwkt5[1])
                    if i != 0 :
                ...
                tablegeom = tablegeom + [copy.deepcopy(lignegeom[:])]
                tablegeomnew = tablegeomnew + [copy.deepcopy(lignegeom[:])]
            ...
            for j in range(len(tablegeom[i])-1):
                ...
                for line_feature2 in cands:
                    if line_feature.geometry().intersects(line_feature2.geometry()):
                        intersection = line_feature.geometry().intersection(line_feature2.geometry()).asWkt()
                        if str(intersection[0:5]) == 'Point' or str(intersection[0:5]) == 'POINT' or str(intersection[0:5]) == 'point':
                            geomwkt2 = intersection.split('(')
                            geomwkt3 = geomwkt2[1].split(')')
                            geomwkt4 = geomwkt3[0].split(' ')
                            x = float(geomwkt4[0])
                            y = float(geomwkt4[1])
                ...
                if len(stockpt) > 0:
                    ...
                    compte = 0
                compte = compte + 1
        v = QgsVectorLayer("LineString?crs="+str(epsg), "Edges", "memory")
        p = v.dataProvider()
  ...
  def calcul2fpgb(self):
        ...
        self.dlg3.ui.comboBox.addItems(fields)
        self.dlg3.ui.comboBox2.addItems(fields)
        self.dlg3.ui.buttonBox.accepted.connect(self.calcul3fpgb)
        self.dlg3.show()
  ...
  def calcul2fid(self):
        ...
        self.dlg3.ui.buttonBox.accepted.connect(self.calcul3fid)
        self.dlg3.show()
  ...
  def calcul2fet(self):
        ...
        self.dlg3.ui.buttonBox.accepted.connect(self.calcul3fet)
        self.dlg3.show()
  ...
  def calcul2fdin(self):
        ...
        self.dlg3.ui.comboBox2.addItems(fields)
        self.dlg3.ui.comboBox3.addItems(fields)
        self.dlg3.ui.buttonBox.accepted.connect(self.calcul3fdin)
        self.dlg3.show()
  ...
  def calcul2fgs(self):
        ...
        self.dlg3.ui.buttonBox.accepted.connect(self.calcul3fgs)
        self.dlg3.show()
  ...
  def calcul2fgc(self):
        ...
        self.dlg3.ui.comboBox_4.addItems(["No","Yes"])
        self.dlg3.ui.comboBox_5.addItems([""]+fields)
        self.dlg3.ui.buttonBox.accepted.connect(self.calcul3fgc)
        self.dlg3.show()
  ...
  def calcul2ftoun(self):
        ...
        self.dlg4.ui.buttonBox.accepted.connect(self.calcul3ftoun)
        self.dlg4.show()
                ...
                if ok3 :
                    ...
                    if t == "Simple" :
                        ...
                        while fit1.nextFeature(feat):
                            ...
                            if geom == [] :
                                ...
                                deltax = float(geom[0][1][0]) - float(geom[0][0][0])
                                deltay = float(geom[0][1][1]) - float(geom[0][0][1])
                                x1 = float(geom[0][0][0])
                                y1 = float(geom[0][0][1])
                                x2 = float(geom[0][1][0])
                                y2 = float(geom[0][1][1])
                            ...
                            v.startEditing()
                            fet = QgsFeature()
                            fet.setGeometry(  QgsGeometry.fromPolyline( [QgsPoint(float(x1),float(y1)), QgsPoint(float(x1) + dx1 + dx2 , float(y1) + dy1 + dy2), QgsPoint(float(x2),float(y2))] ))
                    ...
                    if t == "Optimized" :
                        ...
                        while fit1.nextFeature(feat):
                            ...
                            dist1 = (xmoy - (x1 + dx1 + dx2a)) * (xmoy - (x1 + dx1 + dx2a)) + (ymoy - (y1 + dy1 + dy2a)) * (ymoy - (y1 + dy1 + dy2a))
                            dist2 = (xmoy - (x1 + dx1 + dx2b)) * (xmoy - (x1 + dx1 + dx2b)) + (ymoy - (y1 + dy1 + dy2b)) * (ymoy - (y1 + dy1 + dy2a))
                            if dist1 > dist2 :
                                dx2 = dx2a
                                dy2 = dy2a
                            else :
  ...
  def calcul3f(self):
        ...
        while fit1.nextFeature(feat):
            ...
            v.startEditing()
            fet = QgsFeature()
            fet.setGeometry(  QgsGeometry.fromPointXY(QgsPointXY(float(x1),float(y1))) )
  ...
  def calcul3flc(self):
        ...
        if str(poids)=="Yes":
         ...
         G.add_weighted_edges_from(table)
        t=nx.number_of_nodes(G)
        res = max(nx.connected_components(G), key=len)
        trires = sorted(res)
        v = QgsVectorLayer("LineString?crs="+str(epsg), "Edges", "memory")
        p = v.dataProvider()
        p.addAttributes( [QgsField("ID", QVariant.Int)] )
        ...
        for i in range(len(table)):
            ...
            if test == 0 :
                ...
                idarcnew = idarcnew + 1
        QgsProject.instance().addMapLayer(v)
        v2 = QgsVectorLayer("Point?crs="+str(epsg2), "Nodes", "memory")
        p2 = v2.dataProvider()
        p2.addAttributes( [QgsField("ID", QVariant.Int)] )
  ...
  def calcul3fgc(self):
        ...
        if str(oriente)=="Yes" :
            ...
            if res != [] :
                ...
                for i in range(len(res)):
                    trires = sorted(res[i])
                    v = QgsVectorLayer("LineString?crs="+str(epsg), "Edges"+str(i), "memory")
                    p = v.dataProvider()
                    p.addAttributes( [QgsField("ID", QVariant.Int)] )
                    p.addAttributes( [QgsField("ID2", QVariant.Int)] )
                    p.addAttributes( [QgsField("StartNode", QVariant.Int)] )
                    ...
                    for j in range(len(trires)):
                        v2.startEditing()
                        fet = QgsFeature()
                        fet.setGeometry(  QgsGeometry.fromPointXY(geompt[trires[j]-1]) )
  ...
  def calcul3fdraw(self):
        ...
        if str(indicateur) == "Fruchterman-Reingold force-directed algorithm":
            ...
            nx.draw(Gdraw,pos = positi,node_size=200)
            ax.plot()
        if str(indicateur) == "Kamada Kawai":
            positi = nx.kamada_kawai_layout(Gdraw)
            nx.draw(Gdraw,pos = positi,node_size=200)
            ax.plot()
        ...
        if str(indicateur) == "Circular":
            ...
            nx.draw(Gdraw,pos = positi,node_size=200)
            ax.plot()
        if str(indicateur) == "Spiral":
            positi = nx.spiral_layout(Gdraw)
            nx.draw(Gdraw,pos = positi,node_size=200)
            ax.plot()
        if str(indicateur) == "Force Atlas":
            positi = nx.forceatlas2_layout(Gdraw)
            nx.draw(Gdraw,pos = positi,node_size=200)
            ax.plot()
        self.dlg4.ui.canvas.draw()
        self.dlg4.ui.bouton1.clicked.connect(self.changedraw)
        self.dlg4.ui.bouton2.clicked.connect(self.changedrawcolor)
        self.dlg4.ui.comboBox.addItems(["Fruchterman-Reingold force-directed algorithm", "Kamada Kawai", "Spectral", "Circular", "Spiral", "Force Atlas"])
        self.dlg4.show()
  ...
  def changedrawcolor(self):
        ...
        self.dlg5.ui.bouton1.clicked.connect(self.colorpicker1)
        self.dlg5.ui.bouton3.clicked.connect(self.colorpicker2)
        self.dlg5.ui.buttonBox.accepted.connect(self.editdraw)
        self.dlg5.show()
  ...
  def editdraw(self) :
        ...
        couleur2 = couleur.split(": ")
        couleur3 = couleur2[1].split("}")
        couleurnew = str(couleur3[0])
        couleura = str(self.dlg5.ui.lineedit3.styleSheet())
        couleura2 = couleura.split(": ")
        couleura3 = couleura2[1].split("}")
        ...
        if test1 == "Yes":
            ...
            tfin = [0]  * len(tablech)
            j = list( positi.keys() )
            for i in range(len(tablech)):
                tfin[i] = tablech[ int(j[i]-1) ]
            taille3 = np.array(tfin)
            m = max(taille3)
        ...
        self.dlg4.ui.figure.clear()
        ax = self.dlg4.ui.figure.add_subplot(111)
        nx.draw(Gdraw,pos = positi,node_size = taille, node_color = couleurnew, cmap=comaps, edge_color = couleurnewa, width = taille_edge, edge_cmap = comaps_edge, with_labels = wlab, labels = wlab2, font_size = taille_lab )
        ax.plot()
        self.dlg4.ui.canvas.draw()
        self.dlg4.show()

  def calcul3fpgb(self):
        epsg = eLayer.sourceCrs().authid()
        arc_start=self.dlg3.ui.comboBox.currentText()
        ...
        feat = QgsFeature()
        fit = provider.getFeatures()
        v = QgsVectorLayer("Point?crs="+str(epsg), "Nodes", "memory")
        p = v.dataProvider()
        p.addAttributes( [QgsField("ID", QVariant.Int)] )
        p.addAttributes( [QgsField("ID2", QVariant.Int)] )
        v.commitChanges()
        ...
        v2 = QgsVectorLayer("LineString?crs="+str(epsg), "Edges", "memory")
        p2 = v2.dataProvider()
                ...
                if sid[1] == start:
                    ...
                    for fid in enumerate(tablept) :
                        if fid[1] == end :
                            ...
                            test = 1
            if test == 0 :
                tableconfig = tableconfig + [[str(feat.geometry().asWkt()),int(feat.attributes()[champ1]),int(feat.attributes()[champ2]),valconfig]]
  ...
  def calcul3fdin(self):
        ...
        while fit1.nextFeature(feat):
            ...
            geom_arc=geom_arc+[feat.geometry().asWkt()]
        feat2 = QgsFeature()
        table2 = []
        table_noeud = []
        geom_noeud = []
        ...
        while fit2.nextFeature(feat2):
            ...
            geom_noeud = geom_noeud + [feat2.geometry().asWkt()]
        tablenew = []
        vl = QgsVectorLayer("LineString?crs="+str(epsg), "Edges", "memory")
        pr = vl.dataProvider()
        for j in range(field.count()):
        ...
        nod = list(G.nodes())
        vl2 = QgsVectorLayer("Point?crs="+str(epsg2), "Nodes", "memory")
  ...
  def calcul3fgs(self):
        ...
        for i in range(len(valclass)):
            ...
            vl = QgsVectorLayer("LineString?crs="+str(epsg), nom, "memory")
            pr = vl.dataProvider()
            for j in range(field.count()):
                pr.addAttributes( [field[j]] )
            ...
            vl2 = QgsVectorLayer("Point?crs="+str(epsg2), nom2, "memory")
            pr2 = vl2.dataProvider()
            for j in range(field2.count()):
                pr2.addAttributes( [field2[j]] )
            vl2.startEditing()
            ...
            QgsProject.instance().addMapLayer(vl)
            QgsProject.instance().addMapLayer(vl2)
        edge_sup = list(G2.edges())
        node_sup = list(G2.nodes())
        deg = G2.degree()
        vl = QgsVectorLayer("LineString?crs="+str(epsg), "Edges_sup", "memory")
        pr = vl.dataProvider()
        for j in range(field.count()):
            pr.addAttributes( [field[j]] )
        vl.startEditing()
        for i in range(len(edge_sup)):
            ...
            try :
                ...
                pr.addFeatures( [ fet ] )
        vl.commitChanges()
        vl2 = QgsVectorLayer("Point?crs="+str(epsg), "Nodes_Edges_Sup", "memory")
        pr2 = vl2.dataProvider()
        for j in range(field2.count()):
            pr2.addAttributes( [field2[j]] )
        vl2.startEditing()
  ...
  def calcul3ftoun(self):
        ...
        if str(oriente) == "Yes" :
            ...
            if ok1 :
                if testzero == "No" :
                    vln = QgsVectorLayer("LineString?crs="+str(epsg), "Edges_undirected", "memory")
                    pr = vln.dataProvider()
                    vln.startEditing()
  ...
  def calcul3fet(self):
        ...
        for i in range(len(tabadrr)):
            req = "http://photon.komoot.io/api/?q="
        ...
        for i in range(len(attrs)):
            ...
            fet = QgsFeature()
            fet.setGeometry( QgsGeometry.fromPolylineXY( [ QgsPointXY(float(tabxy[int(tabligne[i][0])][0]), float(tabxy[int(tabligne[i][0])][1])) , QgsPointXY(float(tabxy[int(tabligne[i][1])][0]),float(tabxy[int(tabligne[i][1])][1])) ] ))
            fet.setAttributes( att )
            vl.startEditing()
            pr.addFeatures( [ fet ] )
            vl.commitChanges()
