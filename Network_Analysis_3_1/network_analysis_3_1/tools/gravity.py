...
def com_spa(Gr,Gapprox,orie,isol): 
    noeuds = list(Gr.nodes())
    sum1 = sum(dict(nx.degree(Gr,weight='weight')).values())/float(2)
    sum2 = sum(dict(nx.degree(Gapprox,weight='weight')).values())/float(2)
    part = []
    ...
    if isol == "Yes" :
        ...
        for i in range(len(part)):
            if len(part[i]) == 1 :
                edg = list(Gr.edges(int(part[i][0])))
                if str(orie) == "Yes":
                    edg = list(GUN.edges(int(part[i][0]))	)	
                valmax = 0
                for j in range(len(edg)):
                    try :
